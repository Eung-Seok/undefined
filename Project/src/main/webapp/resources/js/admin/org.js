document.addEventListener('DOMContentLoaded', () => {
  const API_URL = (window.APP_CTX || '') + '/admin/departments/tree?includeUsers=true';
    ...
    if (isExpanded) {
      if (hasUsers) {
        ...
        for (const u of node.users) {
          ...
          const empno = (u.empno != null) ? u.empno : u.EMPNO;
          right.textContent = (empno != null) ? String(empno) : '';
  ...
  function matchDept(node, q) {
    ...
    const s = ((node.name || '') + ' ' + (deptno != null ? deptno : '') + ' ' + (id != null ? id : '')).toLowerCase();
    return s.includes(q);
  }
